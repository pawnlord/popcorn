...
void test_malloc() {
    ...
    kasserteq(current_block->allocated, 0,
              "test_malloc: start block is allocated");
    kasserteq((uint32_t)current_block, (uint32_t)block_start,
              "test_malloc: current block != start block");
    void *tmp = malloc(25);
    kassertneq((uint32_t)current_block, (uint32_t)block_start,
               "test_malloc: current block == start block");
    kasserteq((uint32_t)current_block, (uint32_t)block_start->next,
              "test_malloc: current block != start block");
    kasserteq(block_start->free, 0, "test_malloc: starting block is free");
    kasserteq(block_start->allocated, 1,
              "test_malloc: starting block not allocated");
    kasserteq(block_start->sz, 25, "test_malloc: block size !=  25");
    kasserteq(block_start->sz + sizeof *block_start,
              ((uint32_t)current_block) - ((uint32_t)block_start),
              "test_malloc: distance between blocks unexpected");
    void *tmp2 = malloc(25);
    free(tmp);
    kasserteq(current_block->free, 1, "test_malloc: freed block is not free");
    kasserteq(current_block->allocated, 1,
              "test_malloc: freed block is not allocated");
    kasserteq(current_block->sz, 25,
              "test_malloc: freed block is not the correct size");
    kasserteq(current_block->sz,
              ((uint32_t)tmp2) - ((uint32_t)tmp) - sizeof *current_block,
              "test_malloc: current block not in the correct location");
    kasserteq((uint32_t)current_block->next,
              ((uint32_t)tmp2) - sizeof *current_block,
              "test_malloc: freed block does not point to last block");
    kasserteq(((uint32_t)tmp) - sizeof *current_block, (uint32_t)current_block,
              "test_malloc: current block not in the correct location");
    void *tmp3 = malloc(25);
    ...
    while (ptr == 0) {
        if (!current_block->allocated) {
            if (current_block->next != (BlockInfo *)sbrk(0)) {
                current_block->next = (BlockInfo *)sbrk(
                    sizeof *block_start);  // sbrk has been moved, but
            ...
            if (size <= current_block->sz - sizeof *current_block) {
                current_block->sz = size;
                BlockInfo *next =
                    (BlockInfo *)(((uint32_t)current_block) +
                                  current_block->sz + sizeof *current_block);
                next->next = current_block->next;
                next->sz = current_block->sz - size - sizeof *next;
                next->free = 1;
                next->allocated = 1;
                ptr = (BlockInfo *)(((uint32_t)current_block) +
