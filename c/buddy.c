...
void get_level(int size, int *level, int *min_level, int *min_blocks) {
    ...
    *level = 0;
    ...
    size_t temp_sz = size;
    while (temp_sz >> 1 != 0) {
        /* if the tail bits are not empty, then it must round up to the next
           level e.g 10000 is going to be level 4, but 10001 should be level 5
           as it can not fit into a 4 block
        */
        if (temp_sz & 1) {
            if (round_up == 0) {
                *min_level = *level;
...
int request_page(size_t size, uint32_t page_table[], int idx) {
    ...
    for (curr_level = level - 1; curr_level >= 0; curr_level--) {
        ...
        int cbyte = cbit / 8;
        cbit %= 8;
        int cblocks = (min_blocks * (curr_level <= min_level))
